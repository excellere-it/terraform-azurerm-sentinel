...
.PHONY: help docs fmt validate test test-quick test-terraform clean init plan deploy destroy pre-commit security-scan lint dev check info
...
test: fmt validate docs test-terraform ## Run full test suite (format, validate, docs, tests)
	@echo "$(GREEN)========================================$(NC)"
	@echo "$(GREEN)✓ All tests passed!$(NC)"
	@echo "$(GREEN)========================================$(NC)"

test-quick: test-terraform ## Run tests without pre-checks
...
info: ## Display project information
	...
	@echo "$(CYAN)========================================$(NC)"
	@echo "$(GREEN)Module Name:$(NC) terraform-azurerm-sentinel"
	@echo "$(GREEN)Terraform Version:$(NC) >= 1.13.4"
	@echo "$(GREEN)Provider:$(NC) azurerm ~> 4.0"
